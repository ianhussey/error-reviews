...
colnames(original_dat_exp_2)

dat <- original_dat_exp_2 |>
  select(id, anxiety, eat, apathy, alcohol, zung, ocir, leb, iq, bis, 
schizo) 

dat |>
  mutate(zung_cutoff = ifelse(zung >= 70, TRUE, FALSE),
         stai_cutoff = ifelse(anxiety >= 45, TRUE, FALSE),
         audit_cutoff = ifelse(alcohol >= 16, TRUE, FALSE),
         ocir_cutoff = ifelse(ocir >= 41, TRUE, FALSE)) |>
  select(id, zung_cutoff, stai_cutoff, audit_cutoff, ocir_cutoff) |>
  pivot_longer(cols = -id,
               names_to = "scale",
               values_to = "classification") |>
  group_by(scale) |>
  summarize(proportion = mean(classification))
...
dat2 <- original_dat_exp_2 |>
  select(id, AD, Compul, SW) 
