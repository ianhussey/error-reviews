...
source(here::here("../../secret_vars.R"))
```
...
This piece of code will look like this (commented out so you know you could not run it, as we didn't load the functions yet.):
<!-- this code block is just for demonstration, it does not run, because the functions are not loaded yet. -->
...
retrieve_journal_metadata <- function(issn_list) {
  ...
  for (issn in issn_list){
    ...
    if(is.null(journal_citestats)){
      message(paste0("no citestats for journal with ISSN: ", issn, ". Skipping..."))
      next
    }
    ...
    journal_citestats_df$journal_issn <- ifelse(is.null(journal_metadata_body$`prism:issn`), gsub("-", "", journal_metadata_body$`prism:eIssn`), gsub("-", "", journal_metadata_body$`prism:issn`))
...
```{r, class.source = "fold-show"}
# create a data frame of the list of queried items
query_and_process <- function(pubyear_start, pubyear_end, issn_list, max_count = 1e5, query_type = c("issn", "direct"), direct_query = "") {

  # create empty data frame to store results
  processed_df_final <- NULL
  if(query_type == "direct"){
    issn_list <- c("direct_query")
  }
  ...
  for(issn in issn_list){
    ...
    } else if(query_type == "direct"){
      query_return <- rscopus::scopus_search(query = paste0(direct_query, "AND PUBYEAR > ", (pubyear_start-1), " AND PUBYEAR < ", (pubyear_end+1)), view="COMPLETE", max_count = max_count, api_key = scopus_key, count = 25, verbose = FALSE, wait_time = 1)
    }
  ...
  if(query_type == "issn"){
    ...
    issn_direct <- unique(processed_df_final$journal_issn)
    journal_metadata <- retrieve_journal_metadata(issn_direct)
  }
...
Note that This code will only run from within University VPN or on campus, as the "COMPLETE" Scopus API is only accessible from there.
...
direct_query <- "all(psychotherapy)" # for more information on how to construct these terms see https://schema.elsevier.com/dtds/document/bkapi/search/SCOPUSSearchTips.htm

direct_query_test_data <- query_and_process(
  pubyear_start = 2024,
  pubyear_end = 2024,
  max_count = 100,
  query_type = "direct",
...
#save to csv
write.csv(PS_2024, here::here("data/PS_2024_metadata.csv"))
```
...
head_df <- data.frame(lapply(head(PS_2024), function(x) {
  if(is.character(x)) {
...
```{r, eval = FALSE, class.source = "fold-show"}
gs4_auth(email = my_gmail) # my_gmail is stored in secret_vars.R
```
...
# now lets have a look at the Spreadsheet ID 
gs_worksheet
# save the value shown next to ID to secret_vars.R as google_sheet_id for later use 

# save local R data frame copy
saveRDS(journal_dfs, file = here::here(paste0("publication_data_", str_replace_all(Sys.time(), ":", ""), ".rds")))
```
